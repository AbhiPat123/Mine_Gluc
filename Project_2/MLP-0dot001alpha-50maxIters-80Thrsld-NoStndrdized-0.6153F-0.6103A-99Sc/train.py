...
def main():
    ...
    mat_NMDPat_1 = getNMDFeatMat(insCarbDT_1, cgmValDT_1, cgmDataPat_1, threshold)
    mat_NMDPat_2 = getNMDFeatMat(insCarbDT_2, cgmValDT_2, cgmDataPat_2, threshold)
    ...
    X_tr_full = np.row_stack( (X_tr, X_ts) )
    y_tr_full = np.row_stack( ( np.reshape(y_tr, (-1,1)), np.reshape(y_ts, (-1,1)) ) )
    ...
    model = MLPClassifier(alpha=0.001, max_iter=50)#linear_model.SGDClassifier()#MLPClassifier(alpha=0.0001, activation='logistic', max_iter=500)
    model.fit(X_tr_full, y_tr_full.ravel())
    # save the fitted model to pickle
    filename = 'model.pkl'
    pickle.dump(model, open(filename, 'wb'))

    print("LEARNED MODEL SAVED IN model.pkl!")
