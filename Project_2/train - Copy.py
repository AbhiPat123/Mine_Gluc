...
def main():
    ...
    #-------------------------------------    MODEL TRAINING (SVM)
    model = DecisionTreeClassifier(max_depth=7)#RandomForestClassifier(max_depth=5, n_estimators=10, max_features='auto')
...
def feat_extr(data_mat):
    ...
    for idx, row in enumerate(data_mat):
        ...
        # fft
        fft_vals = fft( nonNARow )#np.fft.fft( nonNARow )
